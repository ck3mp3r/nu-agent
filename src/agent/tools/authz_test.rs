...
fn async_ask_waits_for_matching_decision_before_resolving() {
    ...
    let outcome = crate::agent::protocol::permission::submit_active_permission_decision(
        request_id.clone(),
...
fn ask_event_carries_pre_authorize_display_context_when_provided() {
    ...
    let ask_context = AskContext {
        pre_authorize_display: Some(crate::agent::protocol::event::ToolDisplay {
...
fn submit_active_permission_decision_without_active_sender_is_ignored() {
    ...
    assert_eq!(
        outcome,
        crate::agent::protocol::permission::SubmitOutcome::Ignored {
...
fn permission_request_token_rejects_mismatched_rule_identity() {
    let controller =
        crate::agent::protocol::permission::PermissionController::new(Duration::from_secs(1));
    let request = crate::agent::protocol::permission::PermissionRequest {
