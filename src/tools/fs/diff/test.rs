...
fn deterministic_diff_core_insert_delete_and_noop_behavior() {
    ...
    let inserted = compute_edit_unified_diff_bounded(path, "alpha\n", "alpha\nbeta\n", DiffBounds::default());
    assert!(inserted.text.contains("+beta\n"));
    assert_eq!(inserted.stats.insertions, 1);
    assert_eq!(inserted.stats.deletions, 0);

    let deleted = compute_edit_unified_diff_bounded(path, "alpha\nbeta\n", "alpha\n", DiffBounds::default());
...
fn newline_and_eof_semantics_include_no_newline_markers_and_keep_line_endings() {
    ...
    let eof_added = compute_edit_unified_diff_bounded(path, "alpha", "alpha\n", DiffBounds::default());
    assert!(eof_added.text.contains("\\ No newline at end of file\n"));

    let eof_removed = compute_edit_unified_diff_bounded(path, "alpha\n", "alpha", DiffBounds::default());
    assert!(eof_removed.text.contains("\\ No newline at end of file\n"));

    let crlf_change = compute_edit_unified_diff_bounded(path, "a\r\nb\r\n", "a\r\nc\r\n", DiffBounds::default());
    assert!(crlf_change.text.contains("-b\r\n"));
    assert!(crlf_change.text.contains("+c\r\n"));

    let mixed_eol = compute_edit_unified_diff_bounded(path, "a\r\nb\n", "a\r\nc\n", DiffBounds::default());
