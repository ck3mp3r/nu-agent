...
use super::{
    ...
    tool_handler::{self, McpToolRegistry, ToolSource},
    ui::event::UiEvent,
};
...
impl ConversationRuntime for AgentConversationRuntime {
    fn execute_turn<U: ProgressUi>(
        ...
        let mut merged_prompt = super::merge_prompt_with_context(&prompt, context.as_deref());
        ...
        while !llm_response.tool_calls.is_empty() && tool_turn < max_tool_turns {
            ...
            let tool_results = self.runtime.block_on(tool_handler::handle_tool_calls(
