    ...
    H: AskApprovalHook,
{
    fn resolve<S: PermissionEventSink>(
        ...
        let pre_authorize_output = crate::tools::handler::pre_authorize::pre_authorize_tool_call(
            &tool_call,
            source.clone(),
            self.engine,
        );

        let flow_context = crate::tools::handler::AuthorizationFlowContext {
            ask_context: pre_authorize_output.ask_context.clone(),
        };

        // 5. Enforce authorization (includes session grant checks and ask prompts)
        let denied_details = crate::tools::handler::enforce_authorization_for_tool_call(
            &tool_call,
            source,
            self.permissions,
            self.grant_cache,
