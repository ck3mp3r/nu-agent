...
async fn glob_finds_matching_files() {
    ...
    let result = GlobTool::execute(&args, dir.path(), &Bus::new())
        .await
        .unwrap();
...
#[tokio::test]
async fn glob_returns_empty_for_no_match() {
    let dir = tempdir().unwrap();
    std::fs::write(dir.path().join("foo.rs"), "").unwrap();
...
#[tokio::test]
async fn glob_searches_recursively() {
    let dir = tempdir().unwrap();
...
async fn glob_invalid_pattern_returns_validation_error() {
    ...
    let result = GlobTool::execute(&args, dir.path(), &Bus::new()).await;
    match result {
