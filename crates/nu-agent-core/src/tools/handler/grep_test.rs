...
#[tokio::test]
async fn grep_returns_empty_when_no_match() {
    let dir = tempdir().unwrap();
...
async fn grep_case_insensitive_flag() {
    ...
    let sensitive_res = GrepTool::execute(&sensitive, dir.path(), &Bus::new())
        .await
        .unwrap();
    ...
    let insensitive_res = GrepTool::execute(&insensitive, dir.path(), &Bus::new())
        .await
        .unwrap();
...
#[tokio::test]
async fn grep_glob_filter_limits_files() {
    let dir = tempdir().unwrap();
    ...
    let rs_res = GrepTool::execute(&rs_only, dir.path(), &Bus::new())
        .await
        .unwrap();

    let txt_only = serde_json::json!({"pattern": "needle", "path": dir.path().to_str().unwrap(), "glob": "*.txt"});
    let txt_res = GrepTool::execute(&txt_only, dir.path(), &Bus::new())
        .await
        .unwrap();
...
#[tokio::test]
async fn grep_respects_max_results_and_sets_truncated() {
    let dir = tempdir().unwrap();
    let content = (1..=10)
    ...
    let capped_res = GrepTool::execute(&capped, dir.path(), &Bus::new())
        .await
        .unwrap();
    ...
    let uncapped_res = GrepTool::execute(&uncapped, dir.path(), &Bus::new())
        .await
        .unwrap();
...
async fn grep_invalid_regex_returns_validation_error() {
    ...
    let err = GrepTool::execute(&args, dir.path(), &Bus::new())
        .await
        .unwrap_err();
...
#[tokio::test]
async fn grep_searches_subdirectories_recursively() {
    let dir = tempdir().unwrap();
...
async fn grep_skips_binary_files_without_error() {
    ...
    let result = GrepTool::execute(&args, dir.path(), &Bus::new()).await;
    assert!(
        result.is_ok(),
