...
use crate::bus::{
    Bus, CancelEvent, CompactionEvent, ExternalEvent, LlmEvent, PermissionEvent, SessionEvent,
    ...
    F: FnOnce(mpsc::Sender<OrchestratorEvent>) + Send + 'static,
{
    ...
    // Worker event bridge: forward worker UiEvents to the corresponding bus channels.
        ...
        while let Some(event) = worker_event_rx.recv().await {
            match event {
                UiEvent::ToolStart {
                    ...
                    let _ = worker_bus.tool().send(ToolEvent::Start {
                        name,
                        source,
                        arguments,
                    });
                ...
                UiEvent::ToolEnd {
                    ...
                    let _ = worker_bus.tool().send(ToolEvent::End {
                        name,
                        source,
                        arguments,
                        success,
                ...
                UiEvent::LlmStart => {
                    let _ = worker_bus.llm().send(LlmEvent::Start);
                }
                UiEvent::LlmEnd {
                    response_chars,
                    tool_calls,
                    ...
                    let _ = worker_bus.llm().send(LlmEvent::End {
                        response_chars,
                        tool_calls,
                        input_tokens,
                        output_tokens,
                ...
                UiEvent::AssistantMessage { text } => {
                    let _ = worker_bus.llm().send(LlmEvent::AssistantMessage { text });
                }
                UiEvent::Warning { message } => {
                    let _ = worker_bus.warning().send(WarningEvent::Message { message });
                }
                UiEvent::TurnError { message } => {
                    let _ = worker_bus
                        .warning()
                        .send(WarningEvent::TurnError { message });
                }
                UiEvent::CompactionStarted { source } => {
                    let _ = worker_bus.compaction().send(CompactionEvent::Started {
                        source: Some(source),
                    });
                }
                UiEvent::CompactionSummaryChunk {
                    ...
                    let _ = worker_bus.compaction().send(CompactionEvent::SummaryChunk {
                        source,
                        delta,
                        aggregated,
                    });
                ...
                UiEvent::CompactionTriggered {
                    ...
                    let _ = worker_bus.compaction().send(CompactionEvent::Triggered {
                        source,
                        summarized_count,
                        kept_recent_count,
                        summary_preview,
                ...
                UiEvent::CompactionFailed { source, message } => {
                    let _ = worker_bus
                        .compaction()
                        .send(CompactionEvent::Failed { source, message });
                }
                UiEvent::Completed { tool_calls } => {
                    let _ = worker_bus
                        .turn()
                        .send(TurnEvent::TurnCompleted { tool_calls });
                }
                UiEvent::Tick => {}
                UiEvent::PermissionRequested {
                    ...
                    let _ = worker_bus.permission().send(PermissionEvent::Requested {
                        request_id,
                ...
                UiEvent::PermissionDecisionSubmitted {
                    ...
                    let _ = worker_bus
                        .permission()
                        .send(PermissionEvent::DecisionSubmitted {
                            request_id,
                            decision,
                            matched_rule_identity,
                        });
                ...
                UiEvent::PermissionDecisionTimedOut { request_id } => {
                    let _ = worker_bus
                        .permission()
                        .send(PermissionEvent::DecisionTimedOut { request_id });
                }
                UiEvent::PermissionDecisionIgnored { request_id, reason } => {
                    let _ = worker_bus
                        .permission()
                        .send(PermissionEvent::DecisionIgnored { request_id, reason });
                }
            }
        }
    });
