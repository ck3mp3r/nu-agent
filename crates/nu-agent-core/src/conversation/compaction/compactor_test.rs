...
use crate::bus::{Bus, CompactionEvent, CompactionRx};
...
fn last_request_prompt(model: &MockCompletionModel) -> Result<String> {
    let requests = model.requests();
    let last = requests.last().ok_or("at least one request recorded")?;
    let prompt = last.chat_history.last().ok_or("chat_history non-empty")?;
    match prompt {
        Message::User { content } => Ok(content
            .iter()
            .filter_map(|c| match c {
                UserContent::Text(t) => Some(t.text.clone()),
                _ => None,
            ...
            .collect::<Vec<_>>()
            .join(" ")),
        Message::System { content } => Ok(content.clone()),
...
async fn recv_until_failed(rx: &mut CompactionRx) -> Result<String> {
    loop {
        let event = rx
            .recv()
            .await
...
#[tokio::test]
async fn artifact_is_user_message_with_required_header_and_summary() -> Result<()> {
    let bus = Bus::default();
...
#[tokio::test]
async fn carry_over_summary_is_prepended_into_summarizer_input() -> Result<()> {
    let bus = Bus::default();
    ...
    let first_artifact = compactor
        ...
        .await
        .map_err(|e| format!("first compact should succeed: {e:?}"))?;

    let _ = compactor
        .compact("conv-1", &evicted, Some(&first_artifact), "test_source")
        .await
        .map_err(|e| format!("second compact should succeed: {e:?}"))?;
...
#[tokio::test]
async fn started_and_summary_chunk_events_are_emitted_on_bus() -> Result<()> {
    let bus = Bus::default();
    ...
    let started = rx.recv().await.map_err(|_| "Started event emitted")?;
    assert!(
        matches!(started, CompactionEvent::Started { .. }),
        "first event should be Started, got {started:?}"
    );

    let chunk = rx.recv().await.map_err(|_| "SummaryChunk event emitted")?;
    assert!(
...
#[tokio::test]
async fn max_bytes_cap_truncates_body_at_utf8_boundary_preserving_header() -> Result<()> {
    let bus = Bus::default();
    ...
    let body = text
        ...
        .ok_or("truncated text has header and marker")?;
    assert!(
...
#[tokio::test]
async fn summarizer_error_maps_to_memory_error_backend() -> Result<()> {
    let bus = Bus::default();
    let model = MockCompletionModel::from_stream_turns([[
...
#[tokio::test]
async fn set_model_swaps_the_model_used_by_the_next_compact_call() -> Result<()> {
    let bus = Bus::default();
    ...
    let artifact = compactor
        .compact("conv-1", &evicted, None, "test_source")
        .await
        .map_err(|e| format!("compact after set_model should succeed: {e:?}"))?;
...
#[tokio::test]
async fn from_shared_model_compactor_uses_model_swapped_on_the_shared_arc() -> Result<()> {
    // -- Setup & Fixtures
    let bus = Bus::default();
    ...
    *shared_arc.lock().expect("shared model mutex not poisoned") =
    ...
    let artifact = compactor
        .compact("conv-1", &evicted, None, "test_source")
        .await
        .map_err(|e| format!("compact after set_model on the shared Arc should succeed: {e:?}"))?;
