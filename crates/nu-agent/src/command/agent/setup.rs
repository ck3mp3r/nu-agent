...
use nu_agent_core::tools::closure::ClosureRegistry;
...
pub(crate) struct RegisterToolsInput<'a> {
    ...
    pub(crate) available_agents: &'a [nu_agent_core::protocol::persona::PersonaSummary],
...
pub(crate) fn register_tools(input: RegisterToolsInput<'_>) -> Result<SetupResult, LabeledError> {
    ...
    use nu_agent_core::hook::adapter::builtin::adapt_builtins;
    use nu_agent_core::hook::adapter::closure::adapt_closures;
    ...
    let log_dir = nu_agent_core::utils::xdg::data_dir()
    ...
    let audit_logger = std::sync::Arc::new(nu_agent_core::tools::audit::AuditLogger::new(log_path));
    let tool_executor = nu_agent_core::tools::executor::ToolExecutor::new(
    ...
    let (orchestrator_state, orchestrator_mailbox_rx) = if is_orchestrator {
        ...
        let registry = std::sync::Arc::new(tokio::sync::RwLock::new(
            nu_agent_core::mailbox::AgentRegistry::new(),
        ));
        ...
        let (tokio_tx, mut tokio_rx) =
            tokio::sync::mpsc::channel::<nu_agent_core::mailbox::ServerFrame>(64);
        ...
        let (std_tx, std_rx) =
            std::sync::mpsc::channel::<nu_agent_core::mailbox::IncomingMessage>();
        runtime.spawn(async move {
            while let Some(frame) = tokio_rx.recv().await {
                if let nu_agent_core::mailbox::ServerFrame::Message {
                    from,
                    message,
                    kind,
                } = frame
                {
                    ...
                    if std_tx
                        .send(nu_agent_core::mailbox::IncomingMessage {
                            from,
                            message,
                            kind,
                        })
        ...
        let state = Some(std::sync::Arc::new(std::sync::Mutex::new(
            nu_agent_core::tools::handler::spawn_agent::OrchestratorState {
                agent_identity: messaging_identity.clone(),
                ..nu_agent_core::tools::handler::spawn_agent::OrchestratorState::new(
                    registry,
                    cwd_path.clone(),
                )
            },
    ...
    let (broker_sender, mailbox_rx, parent_name) = if let Some(flags) = broker_flags {
        ...
        let client = runtime
            .block_on(async {
                nu_agent_core::mailbox::BrokerClient::connect(&flags.socket_path, &flags.token)
                    .await
            })
        ...
        runtime.spawn(async move {
            loop {
                match receiver.recv().await {
                    Ok(nu_agent_core::mailbox::ServerFrame::Message {
                        from,
                        message,
                        kind,
                    }) => {
