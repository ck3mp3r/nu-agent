...
fn mcp_server_toggle_and_counts_follow_selected_row() -> Result<()> {
    ...
    assert_eq!(state.status.mcp_counts(), (2, 1, 1, 0));

    state.status.mcp_panel_move_down();
    assert!(state.status.queue_selected_mcp_toggle_request());

    assert_eq!(state.status.mcp_counts(), (2, 1, 1, 0));
    assert_eq!(
        state.status.selected_mcp_server_state(),
    ...
    assert_eq!(state.status.mcp_counts(), (2, 2, 0, 0));
    Ok(())
}
...
fn enabling_failed_server_queues_enable_and_can_transition_to_failed_on_outcome() -> Result<()> {
    ...
    assert_eq!(state.status.mcp_counts(), (1, 0, 0, 1));
    Ok(())
}
...
-> Result<()> {
    ...
    assert_eq!(request.server_name, "gh");
    assert!(!request.enable);
    assert_eq!(state.status.mcp_counts(), (1, 0, 1, 0));
    Ok(())
}
...
fn failed_server_reason_round_trips_through_state_query() {
    ...
    let failed = state.status.failed_mcp_servers_with_reasons();
