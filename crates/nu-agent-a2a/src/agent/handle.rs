...
///
/// Created by [`super::builder::AgentBuilder::build`]. The handle provides
/// access to the server, completion events, task store, and graceful shutdown.
pub struct AgentHandle {
    pub server: A2aServer,
    pub completion_tx: Option<mpsc::Sender<A2aCompletionEvent>>,
    pub(super) completion_rx: Option<mpsc::Receiver<A2aCompletionEvent>>,
    pub(super) task_cancel_rx: Option<mpsc::UnboundedReceiver<String>>,
    pub(super) discovery: Arc<Mutex<PeerDiscoveryImpl>>,
    // Private — held for A2aToolContext construction.
    pub(super) client: A2aClient,
    pub(super) card: AgentCard,
    ...
    pub(super) card_handle: Option<Arc<RwLock<AgentCard>>>,
    pub(super) cache: Arc<PeerCache>,
    /// The mesh key used for discovery isolation.
    pub(super) mesh_key: String,
    /// Token to cancel the periodic mDNS re-registration task.
    pub(super) reregister_token: Option<CancellationToken>,
}
